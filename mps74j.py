...
def analyze_MPS(A, axs, color):
    ...
    MSzMr = np.dot(MSzM, v_right)
    MSpMr = np.dot(MSpM, v_right)
    MSmMr = np.dot(MSmM, v_right)
    Er = np.dot(E, v_right)

    lEr = np.dot(v_left, Er)

    ### one-point functions:
    meanSz = np.dot(v_left, MSzMr) / lEr
    meanSx = 0.5 * np.dot(v_left, MSpMr + MSmMr) / lEr

    print('One-point functions: <Sz>=', np.real(meanSz), ' <Sx>=', np.real(meanSx))

    ### two-point functions:
    r_max = 100
    corr = np.zeros([r_max, 3])

    lOz_string = np.dot(v_left, MSzM)
    lOp_string = np.dot(v_left, MSpM)
    lOm_string = np.dot(v_left, MSmM)
    lE_string = np.dot(v_left, E)

    for l in range(1, r_max):
        denominator = np.dot(lE_string, Er)

        # zz:
        corr[l, 0] = np.real(np.dot(lOz_string, MSzMr) / denominator)
        # pm:
        corr[l, 1] = np.real(np.dot(lOp_string, MSmMr) / denominator)
        # mp:
        corr[l, 2] = np.real(np.dot(lOm_string, MSpMr) / denominator)

        lE_string = np.dot(lE_string, E)
        lOz_string = np.dot(lOz_string, E)
        lOp_string = np.dot(lOp_string, E)
        lOm_string = np.dot(lOm_string, E)
    #print('corr:', corr)
