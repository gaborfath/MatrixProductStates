...
class MPSModel(Model):
    def __init__(self, k, epochs, A0, learning_rate, axs, max_iter, result_log):
        super().__init__()
    ...
    def n_bond_energy(self, A):
        ...
        if (self.counter < 100 and self.counter % 10 == 0) or (self.counter >= 100 and self.counter % 50 == 0):
            ...
            row = {'step': self.counter, 'runtime': (toc - tic).total_seconds(),
                   'chi': self.chi, 'lowest_energy':self.lowest_energy,
                   'delta_energy':energy.numpy() - self.en_inf_fo}
    ...
    if 1: # MPS calculation
        ...
        for i_chi in range(8):
            ...
            if i_chi < 7:
                ...
                max_iter = 4000
            model = MPSModel(k, epochs_list[i_chi], A0, learning_rate, axs, max_iter, result_log)
